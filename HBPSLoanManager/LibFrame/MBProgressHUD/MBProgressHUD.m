...
+ (MB_INSTANCETYPE)showGaryHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    if(view){
        MBProgressHUD *hud = [[self alloc] initWithView:view];
        garyView =[[UIView alloc] initWithFrame:CGRectMake(0, 0, kSCREEN_WIDTH, kSCREEN_HEIGHT)];
        garyView.backgroundColor =[UIColor grayColor];
        [garyView addSubview:hud];
...
+ (BOOL)hideGaryHUDForView:(UIView *)view animated:(BOOL)animated {
